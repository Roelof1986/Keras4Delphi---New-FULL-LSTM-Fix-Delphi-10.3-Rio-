...
//.      function TryGetItem(const name: string; out value: TValue) : Boolean; // --
    public
       ...
       class var UseStateful : Boolean;
       class var UseReturnSequences : Boolean;
       class var UseBatchInputShape: array of Integer;

       // FINAL FIX 15-6-2025 - end
...
//       PyObject : TPythonObject;  // 27-10-2024 : Create re-enabled
(*       constructor Create(py: PPyObject); overload;
       constructor Create(py: TPythonObject); overload;
...
// --
(*function TBase.TryGetItem(const name: string; out value: TValue) : Boolean;
var i : Integer;
begin
  for i := 0 to Parameters.Count - 1 do
    if Parameters[i].Key = name then
...
begin
  ...
  begin
      ...
      //  FIX 14-6-2025
(* .      if item.Key = 'return_sequences' then
      begin

//        kwargs[item.Key] := GetPythonEngine.PyBool_FromLong(1);

        GetPythonEngine.PyDict_SetItemString(kwargs.Handle, 'return_sequences', GetPythonEngine.PyBool_FromLong(1));
  ...
  // Hardcoded via class vars
  if item.Key = 'return_sequences' then
  begin
    if {TLSTM.}UseReturnSequences then
      GetPythonEngine.PyDict_SetItemString(kwargs.Handle, 'return_sequences', GetPythonEngine.PyBool_FromLong(1))
    else
      GetPythonEngine.PyDict_SetItemString(kwargs.Handle, 'return_sequences', GetPythonEngine.PyBool_FromLong(0));
    Continue;
  end;

  if item.Key = 'stateful' then
  begin
    if {TLSTM.}UseStateful then
      GetPythonEngine.PyDict_SetItemString(kwargs.Handle, 'stateful', GetPythonEngine.PyBool_FromLong(1))
    else
      GetPythonEngine.PyDict_SetItemString(kwargs.Handle, 'stateful', GetPythonEngine.PyBool_FromLong(0));
    Continue;
  end;

  if item.Key = 'batch_input_shape' then
  begin
    if Length({TLSTM.}UseBatchInputShape) = 3 then
      GetPythonEngine.PyDict_SetItemString(kwargs.Handle, 'batch_input_shape',
        TPyTuple.Create([
          {TLSTM.}UseBatchInputShape[0],
          {TLSTM.}UseBatchInputShape[1],
          {TLSTM.}UseBatchInputShape[2]
        ]).Handle);
    Continue;
  end;
...
(*constructor TStringOrInstance.Create(py: PPyObject); // 27-10-2024 : all Create re-enabled
begin
    TPythonObject.Create(py);
//    ZeroMemory(@Result, SizeOf(TStringOrInstance));
//    Result.PyObject := TPythonObject.Create( py );
end;
